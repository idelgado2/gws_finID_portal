...
As of August 2019, there are three main isses that need to be resolved. First, when the user initiates the first fin submission in one session a temporary csv file is created, once the user clicks on 'Review Fins' the R script will search for this tempoarary csv file and load it into the editable table. If the user clicks on 'Review Fins' before the first fin upload there will be no tempoaray file created and the site will crash. Conversely, when the user submits the final table to the server by clicking 'Submit to Server' the script will delete the tempoary csv file. Thus, if the user inputs data but never submits to the server and the user returns to the site the old data located in the temporary file will be loaded again in editable table. A work around would be to simply delete the unnecessary rows befor submitting. Seondly, when the user inputs a fin into the site and clicks on 'mas fins?/add fin' the site will keep the previous photo uploaded on the screen. Simply upload the new photo and the photo will change. Lastly, when the user uploads the final table to the server by clicking on 'Submit to Server', the user will be taken to the first tab to start over, however not all fields will reset leaving the previous information still there. The user can simply reload the entire site by refreshing the page and everything will be reset.
